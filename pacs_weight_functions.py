...
def find_weight_log(coords,wcs,func,minval,maxval):
    ...
    #wcs,interp_fit = load_pacs_image(pacs_file_for_interpolation)
    interp_fit = func
    ...
    if interp_value < minval:
        interp_value = minval
    weight = 10.**(2.*(np.log10(interp_value)-np.log10(minval))/(np.log10(maxval)-np.log10(minval)))
    return weight
...
def test_map_coords():
    ...
    for i in range(len(x_r)):
        for j in range(len(y_r)):
            ...
            weight = find_weight_log(coord,pacs_wcs,fit,0.04,0.25)
            ra_list.append(ra)
            dec_list.append(dec)
            weight_list.append([weight])
    print(np.reshape(np.array(weight_list),-1))
    #print(ra_list,dec_list,weight_list)
    plt.figure(figsize=(12,10))
    plt.scatter(np.reshape(np.array(ra_list),-1),
        np.reshape(np.array(dec_list),-1),
        c=np.log10(np.reshape(np.array(weight_list),-1)),cmap=cm.jet,marker='s',s=12)
    plt.gca().invert_xaxis()
    plt.colorbar()
    plt.savefig("weight_map_serpens.png",fmt='png')
...
def test_map_coords_wcs():
    ...
    for i in range(len(x_r)):
        for j in range(len(y_r)):
            ...
            dec = dec_origin+y_r[j]
            coord = SkyCoord(ra,dec,frame='icrs',unit='deg')
            weight = find_weight_log(coord,pacs_wcs,fit,0.05,2)
            ra_list.append(ra)
            dec_list.append(dec)
            weight_list.append([weight])
    print(np.reshape(np.array(weight_list),-1))
